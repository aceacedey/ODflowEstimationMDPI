...
def OsmRouteFlatten(list_osm_points,ogp):
    ...
    for idx,item in enumerate(list_osm_points):
        ...
        OsmList[idx] = path
        #disList[idx] = dis
    OsmListNoCon = [i[0] for i in itertools.groupby(OsmList)]
    return OsmListNoCon#,disList
...
if 1 == 1:                                 
    ...
    og = ox.graph_from_file('MelbourneShort.osm',bidirectional=False,retain_all=False,simplify =False)
...
scatRoute.to_csv('ScatOsmPointRoutesN.csv')
